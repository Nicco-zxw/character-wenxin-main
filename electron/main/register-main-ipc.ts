...
export function registerMainIpcHandlers(deps: RegisterMainIpcHandlersDeps): void {
  ...
  ipcMain.handle('characterarc:truth-export', async (_event, payload: unknown) => {
    const window = deps.windowManager.getActiveWindow()
    if (!window) {
      return { success: false, canceled: true }
    }
    const projectId = String((payload as { projectId?: string })?.projectId ?? '').trim()
    if (!projectId) throw new Error('缺少 projectId。')

    // P7.6：导出可再生只读投影（当前世界状态 + 逐章摘要账 → Markdown），
    // 由用户选择保存位置（可放入 Git / 供人工审阅）；只读、不回读、不构成真相源。
    const db = await deps.ensureWorkspaceDb()
    const markdown = buildTruthProjectionMarkdown(db, projectId)
    const result = await dialog.showSaveDialog(window, {
      title: '导出世界真相投影（可再生 Markdown）',
      defaultPath: 'current_state.md',
