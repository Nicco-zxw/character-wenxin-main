...
export async function indexChapterSegments(
  ...
  const contentHash = createHash('sha256').update(chapterContent).digest('hex')
  db.exec('BEGIN')
  try {
    db.prepare(
    ...
    const stmt = db.prepare(`
      INSERT INTO story_embeddings (id, project_id, source_type, source_id, chapter_index, text_content, embedding, content_hash, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `)
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      stmt.run(id, projectId, 'chapter_segment', chapterId, chapterIndex, segments[i], buffer, contentHash, timestamp)
    }
    db.exec('COMMIT')
    return 'indexed'
  } catch (error) {
...
export async function indexReferenceNovel(
  ...
  const contentHash = createHash('sha256').update(novelText).digest('hex')
  ...
  for (let i = 0; i < segments.length; i += BATCH) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      stmt.run(id, projectId, 'reference_novel', refId, null, batch[j], buffer, contentHash, timestamp)
    }
  }
}
