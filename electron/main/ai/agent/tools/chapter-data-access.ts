...
// 规范搜索范围（如果没有传输搜索范围，则放回null，表示搜索所有类型）
...
// 自然语言章节引用处理（ 去除 前后空格；中间空白；英文大小写差异；书名号和引号。允许用户输入自然语言的格式差异）
...
// 只进行章节修改，并不会提交到数据库（计算逻辑与之前相似）
// 调用者不使用数据库中的当前正文，而是使用临时内容继续计算，能够实现多步编辑方案
...
// 安全提交（实际提交逻辑由commitChapterEditInDb完成）
