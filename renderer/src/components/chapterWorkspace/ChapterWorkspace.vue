...
const pendingAdoptionMemo = ref('') // P8.5：采用分支后的下一章建议 memo（预填写作备忘，作者在环）

function handleGenerateDraft(): void {
  draftConfigVisible.value = true
}

function handleUseAdoptionMemo(text: string): void {
  pendingAdoptionMemo.value = text?.trim() || ''
}

function handleDraftConfigCancel(): void {
  draftConfigVisible.value = false
  pendingAdoptionMemo.value = ''
}

function handleDraftConfigConfirm(config: FirstDraftConfig): void {
  draftConfigVisible.value = false
  pendingAdoptionMemo.value = '' // 已消费，避免残留到下一章
  aiOpen.value = true
  nextTick(() => {
...
<template>
  ...
  >
    ...
    <ChapterEditorPane
      ...
      @use-adoption-memo="handleUseAdoptionMemo"
    />
    <!-- Panel resize handle -->
    <div
      v-if="aiOpen"
    ...
    <ChapterFirstDraftConfigDialog
      ...
      :initial-memo-hint="pendingAdoptionMemo"
      @confirm="handleDraftConfigConfirm"
      @cancel="handleDraftConfigCancel"
    />
  </section>
</template>
