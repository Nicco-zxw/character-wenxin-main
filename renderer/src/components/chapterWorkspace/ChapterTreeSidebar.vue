...
/** 无大纲直接新建空章节：仅需分卷；默认标题「第N章：新章节」自动带章号（store.createChapter）。 */
function submitCreateEmptyChapter(): void {
  if (!createForm.volumeId) {
    message.warning('请先选择所属分卷')
    return
  }
  const volume = appStore.outlineVolumes.find((v) => v.id === createForm.volumeId)
  if (!volume) {
    message.warning('所选分卷不存在')
    return
  }
  appStore.createChapter(volume.id)
  closeCreateDialog()
  emit('navigate')
  message.success('已新建空章节（默认标题自动带“第N章”，可再编辑）')
}
    ...
    >
      ...
      <template #footer>
        <div class="create-actions">
          ...
          <NButton round strong :disabled="!createForm.volumeId" @click="submitCreateEmptyChapter">
            新建空章节（不绑大纲）
          </NButton>
