...
const props = defineProps<{ show: boolean; initialMemoHint?: string }>()
...
watch(() => props.show, (visible) => {
  ...
  // P8.5：采用分支后的下一章 memo 建议 → 预填「写作备忘」步骤补充要求（作者在环，可编辑）
  const hint = (props.initialMemoHint ?? '').trim()
  if (hint) {
    steps.memo.userPrompt = hint
    expandedStepId.value = 'memo'
  }
})
