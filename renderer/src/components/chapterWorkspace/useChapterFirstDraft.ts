...
import { replacementAccepted, shouldRepairAfterAudit } from '@shared/chapter-workflow'
...
export function useChapterFirstDraft(): {
  ...
  async function streamTask(task: StreamTaskName, context: Record<string, unknown>): Promise<StreamTaskResult> {
    ...
    try {
      // P8.1：角色差异化配置 —— 开关开启时按步套用 AgentProfile（默认关，行为与现状一致）。
      // resolveStepSettings 合并链：全局 settings → DEFAULT_AGENT_PROFILES（内置温度档位）→ 单步。
      // 模型级覆盖（AgentProfile.model）需显式 AgentProfileMap（未来项目级/全局配置）才生效。
      const stepRole = chapterRoleForTask(task)
      // P8.7：启用 = 全局常量 或 项目级 enabled；启用时优先项目配置（缺省角色回退内置档位）
      const agentEnabled = CHAPTER_AGENT_PROFILES_ENABLED || appStore.chapterAgentProfilesEnabled
      const stepSettings = agentEnabled
        ? resolveStepSettings(appStore.appSettings, appStore.chapterAgentProfiles ?? DEFAULT_AGENT_PROFILES, stepRole)
    ...
    try {
      await appStore.runTrackedAiTask(
        ...
        async () => {
          ...
          // P8.7：拉取项目级 Agent 差异化配置（enabled+profiles；默认关=行为不变）
          if (project?.id) {
            await appStore.ensureProjectAgentSettings(project.id)
          }
          ...
          if (fullText) {
            ...
            if (steps.audit.enabled && chapterMemo) {
              ...
              try {
                ...
                if (auditResp?.audit) {
                  ...
                  // P8.8：审计→修复判定收敛到共享单一决策源（@shared/chapter-workflow）
                  if (steps.repair.enabled && shouldRepairAfterAudit(auditResp.audit)) {
                    auditResult.value = null
                    isAuditing.value = false
                    ...
                    try {
                      ...
                      // P8.8：修复文本采用门收敛到共享（长度门 0.5）
                      if (replacementAccepted(repairedText, fullText)) {
            ...
            if (steps.humanize.enabled) {
              try {
                ...
                // P8.8：润色采用门收敛到共享（长度门 0.5）
                if (replacementAccepted(humanizedText, finalText)) {
            ...
            if (finalText) {
              ...
              // P4.2-1：定稿即结算 —— 以最终正文同步世界状态（幂等、仅最新章节、失败不打断写作流程）
              const settleProjectId = project?.id
              const settleChapterId = chapter?.id
              if (settleProjectId && settleChapterId) {
                // contextBridge 边界会做结构化克隆：先经 toIpcPayload 摊平 settings（不能把 Vue 响应式 Proxy 直接传出）
                void window.characterArc.settlementSync(toIpcPayload({
                  projectId: settleProjectId,
                  chapterId: settleChapterId,
                  content: finalText,
                  settings: appStore.appSettings
                })).catch(() => {
                  // 定稿同步失败不打断写作流程；可稍后用编辑器「重试结算」手动补
                })
              }
            }
