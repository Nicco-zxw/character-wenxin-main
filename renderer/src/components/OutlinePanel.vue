...
async function handleExpandVolumeOutline(volume: OutlineVolume): Promise<void> {
  ...
  try {
    const result = await appStore.runTrackedAiTask(
      ...
      () =>
        window.characterArc.reflectiveOutlineGenerate(toIpcPayload({
            settings: appStore.appSettings,
            context: {
              projectTitle: appStore.currentProject?.title,
              projectGenre: appStore.currentProject?.genre,
    ...
    message.success(`已为 ${volume.title} 扩写 ${entries.length} 个大纲子节点${payload.iterations && payload.iterations > 1 ? `（反思 ${payload.iterations} 轮）` : ''}`)
  } catch (error) {
