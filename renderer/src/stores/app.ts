...
export const useAppStore = defineStore('app', () => {
  ...
  // ── P8.7 项目级 Agent 差异化配置（enabled + 六角色 × model/temperature/maxTokens） ──
  // 结构同 @shared/agent-profiles.AgentProfileMap（此处内联类型，避免跨端运行 import）
  type ChapterAgentProfile = { model?: string; temperature?: number; maxTokens?: number }
  type ChapterAgentProfileMap = Partial<Record<
    'memo' | 'draft' | 'audit' | 'repair' | 'humanize' | 'session-note',
    ChapterAgentProfile
  >>
  type ChapterAgentSettings = { enabled: boolean; profiles: ChapterAgentProfileMap }
  const agentSettingsByProject = ref<Record<string, ChapterAgentSettings>>({})
  const chapterAgentSettings = computed<ChapterAgentSettings | null>(() => {
    const pid = selectedProjectId.value
    return pid ? (agentSettingsByProject.value[pid] ?? null) : null
  })
  /** 当前选中项目的 Agent 配置（未加载/未配置 → null，六步回退内置档位） */
  const chapterAgentProfiles = computed<ChapterAgentProfileMap | null>(() => chapterAgentSettings.value?.profiles ?? null)
  /** 当前项目是否启用 Agent 差异化（默认关） */
  const chapterAgentProfilesEnabled = computed<boolean>(() => chapterAgentSettings.value?.enabled ?? false)

  async function ensureProjectAgentSettings(projectId: string): Promise<ChapterAgentSettings> {
    const pid = String(projectId ?? selectedProjectId.value ?? '').trim()
    if (!pid) return { enabled: false, profiles: {} }
    const cached = agentSettingsByProject.value[pid]
    if (cached) return cached
    try {
      const res = await window.characterArc.getProjectAgentSettings(pid)
      const settings = res.success && res.result
        ? { enabled: res.result.enabled === true, profiles: res.result.profiles }
        : { enabled: false, profiles: {} }
      agentSettingsByProject.value = { ...agentSettingsByProject.value, [pid]: settings }
      return settings
    } catch {
  ...
  async function saveProjectAgentSettings(
    projectId: string,
    enabled: boolean,
    profiles: ChapterAgentProfileMap
  ): Promise<void> {
    const pid = String(projectId ?? selectedProjectId.value ?? '').trim()
    if (!pid) return
    try {
      const res = await window.characterArc.saveProjectAgentSettings(pid, { enabled, profiles })
      if (res.success && res.result) {
        agentSettingsByProject.value = {
          ...agentSettingsByProject.value,
          [pid]: { enabled: res.result.enabled === true, profiles: res.result.profiles }
        }
      }
    } catch {
      // 保存失败不阻塞（下次写入可重试）
  ...
  return {
    ...
    updateProject,
    chapterAgentProfiles,
    chapterAgentProfilesEnabled,
    ensureProjectAgentSettings,
