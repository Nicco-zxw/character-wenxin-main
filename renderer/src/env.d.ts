...
declare global {
  ...
  type CharacterArcChapterPostGenerationIssuesPayload = {
    ...
    generatedAt: string
    issues: Array<{
      stage: 'state-delta' | 'vector-index' | 'pipeline' | 'settlement'
  ...
  type CharacterArcSettlementRunView = {
    status: 'skipped' | 'settled' | 'settled_with_warning' | 'rejected' | 'error' | 'rolled_back'
    decision: 'apply' | 'apply_with_warning' | 'reject' | 'retry_observe' | 'skip'
    attempt: number
    reason: string
    issues: Array<{
      category: string
  ...
  type CharacterArcForecastRecordView = {
    id: string
    projectId: string
    baseChapterIndex: number
    baseContentHash: string
    title: string
    status: 'active' | 'selected' | 'expired'
    ...
    /** P8.5：采用后生成的下一章建议 memo */
    adoptionMemo?: { currentTask?: string; suggestedHooks?: string[]; note?: string } | null
    /** P8.5：采用时间 */
    adoptedAt?: string | null
    createdAt: string
    updatedAt: string
  }
  /** P8.7：项目级 Agent 差异化配置（六角色 × model/temperature/maxTokens；与 @shared/agent-profiles.AgentProfileMap 结构一致） */
  type ChapterAgentProfileMap = Partial<Record<
    'memo' | 'draft' | 'audit' | 'repair' | 'humanize' | 'session-note',
    { model?: string; temperature?: number; maxTokens?: number }
  >>

  type CharacterArcSettlementRerunResult = {
    status: 'skipped' | 'settled' | 'settled_with_warning' | 'rejected' | 'error' | 'rolled_back'
    decision: 'apply' | 'apply_with_warning' | 'reject' | 'retry_observe' | 'skip'
    applied: boolean
    reason: string
  ...
  interface Window {
    characterArc: {
      ...
      settlementStatus: (projectId: string, chapterId: string, chapterIndex?: number) => Promise<{
        success: boolean
        result?: CharacterArcSettlementRunView | null
        error?: string
      }>
      settlementRollback: (projectId: string, chapterIndex: number) => Promise<{
        success: boolean
        restored?: number
        error?: string
      }>
      settlementRerun: (payload: {
        projectId: string
        chapterId: string
        content: string
        settings: unknown
        ...
        success: boolean
        result?: CharacterArcSettlementRerunResult
        error?: string
      }>
      settlementSync: (payload: {
        projectId: string
        chapterId: string
        content: string
        settings: unknown
        ...
        success: boolean
        action?: 'deferred' | 'up-to-date' | 'settled' | 'error'
        result?: CharacterArcSettlementRerunResult
        error?: string
      }>
      narrativeForecastCreate: (payload: {
        projectId: string
        chapterId: string
        content: string
        branchCount?: number
        ...
        success: boolean
        result?: CharacterArcForecastRecordView | null
        error?: string
      }>
      narrativeForecastList: (projectId: string) => Promise<{
        success: boolean
        result?: CharacterArcForecastRecordView[]
        error?: string
      }>
      narrativeForecastGet: (projectId: string, id: string) => Promise<{
        success: boolean
        result?: CharacterArcForecastRecordView | null
        error?: string
      }>
      narrativeForecastSelect: (projectId: string, id: string, branchId: string) => Promise<{
        success: boolean
        result?: CharacterArcForecastRecordView | null
        error?: string
      }>
      /** P8.5 采用分支并生成下一章建议 memo（只写 forecast 域） */
      narrativeForecastAdoptMemo: (projectId: string, id: string, branchId: string) => Promise<{
        success: boolean
        result?: {
          record?: CharacterArcForecastRecordView | null
          memo?: { currentTask?: string; suggestedHooks?: string[]; note?: string }
          text?: string
        }
        error?: string
      }>
      /** P8.7 项目级 Agent 差异化配置：读取（enabled + 六角色配置） */
      getProjectAgentSettings: (projectId: string) => Promise<{
        success: boolean
        result?: { enabled: boolean; profiles: ChapterAgentProfileMap }
        error?: string
      }>
      /** P8.7 保存项目级 Agent 差异化配置（返回 sanitize 后结果） */
      saveProjectAgentSettings: (projectId: string, settings: { enabled: boolean; profiles: ChapterAgentProfileMap }) => Promise<{
        success: boolean
        result?: { enabled: boolean; profiles: ChapterAgentProfileMap }
        error?: string
      }>
      reflectiveOutlineGenerate: (payload: {
        settings: unknown
        ...
        success: boolean
        result?: {
          entries: Array<{ title?: string; wordTarget?: string | number; conflict?: string; summary?: string }>
          iterations: number
          passed: boolean
        }
        error?: string
      }>
      /** 反思式局部改写（P8.3）：自评重做；替换选区由渲染层作者在环确认 */
      reflectiveRewrite: (payload: {
        settings: unknown
        sourceText: string
        instruction?: string
        maxIterations?: number
        ...
        success: boolean
        result?: { text: string; iterations: number; passed: boolean }
        error?: string
      }>
      ...
      truthExport: (projectId: string) => Promise<{
        success: boolean
        canceled: boolean
        filePath?: string
        error?: string
